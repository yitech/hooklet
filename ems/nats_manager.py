...
class NatsManager:
    ...
    def __init__(self, nats_url: Optional[str] = None, config_manager: Optional[ConfigManager] = None):
        ...
        self._handlers: Dict[str, Dict[str, MessageHandlerType]] = {}  # {subject: {handler_id: handler_func}}
    ...
    async def register_handler(self, subject: str, handler: MessageHandlerType, handler_id: Optional[str] = None) -> str:
        """
        ...
        Args:
            ...
            handler_id: Optional unique identifier for this handler. If not provided, uses function name.
        ...
        if handler_id in self._handlers[subject]:
            raise ValueError(f"Handler with ID '{handler_id}' already registered for subject '{subject}'")
    ...
    async def unregister_handler(self, subject: str, handler_id: str) -> bool:
        ...
        if (subject not in self._subscriptions or 
            subject not in self._handlers or 
            handler_id not in self._subscriptions[subject]):
